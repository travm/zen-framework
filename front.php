<?php include('partials/header.php'); ?>
<?php include('partials/wvu-header.php'); ?>
<?php include('partials/hsc-header.php'); ?>
<?php include('partials/site-header.php'); ?>

<div id="hero">
    <div id="hero-color"></div>
    <div id="hero-content" class="measure">
        <h1>ZEN FRAMEWORK</h1>
        <p>A opinionated web framework designed to create a consistent structure with a flexible and efficient styling strategy. Greatly inspired by the CSS Zen Garden.</p>
    </div>
</div>

<div id="front" class="measure">

    <div id="col-pri">

        <?php include('blocks/block-news.php'); ?>

    </div>

    <div id="col-core">
        
        <h1>What is the Zen Framework?</h1>
        <p>The Zen Framework is an opinionated set of HTML structures and elements that are designed to remain consistent across a large set of sites. This allows for rapid development within a shared templating environment.</p>
        <p>This is a very opinionated framework and is designed specifically to be able to organize and maintain a large set of clients (200+) while reducing development time when designs need to be refreshed or revisited.</p>
        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Adipisci, deserunt, laborum, praesentium, in sed id officiis tempora consequuntur voluptas eius molestias sint odio obcaecati enim nesciunt numquam cumque optio culpa.</p>
        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Perferendis, voluptate, repellat, inventore tempora eius ullam quis fuga soluta odio pariatur dolorum hic cumque iste commodi ipsam? Porro, est aliquam saepe.</p>
        <p>Consequatur, maiores dolor iure quae rem molestias accusantium earum nihil. Tempora, tenetur sapiente hic rerum! Labore, aut, facere, repudiandae aperiam suscipit at incidunt nostrum nobis velit officiis eius commodi cum!</p>
        <p>Voluptatibus, vero, perferendis, sequi, ut corporis quas nemo nihil neque consequuntur atque consectetur repellat fugiat labore ipsa fuga illo error nobis ratione odio esse harum amet ipsum incidunt voluptates accusamus!</p>
        <p>Facilis, perspiciatis dicta impedit ut ad similique harum temporibus dolorum nobis iste quod magni vero pariatur sit non eligendi facere! Voluptatibus animi velit quibusdam quis sapiente esse quas quod neque!</p>
        <p>Ad tenetur assumenda magni tempora non adipisci reiciendis est quasi. Quisquam, alias, debitis, laboriosam asperiores iste saepe unde praesentium molestias omnis deserunt nam perspiciatis provident perferendis consequatur tempora ex cumque?</p>

    </div>

    <div id="col-sec">
        
        <h2>Hello World</h2>
        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Est aut id natus officiis in hic ad neque. Voluptates, tempore perferendis fugit adipisci dolores repudiandae laudantium deserunt pariatur quidem repellat? Perferendis.</p>
        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Assumenda, doloribus, quam quisquam ab sequi aliquam quos tempora possimus dolore unde commodi laboriosam iure eligendi at pariatur. Nisi, perspiciatis mollitia doloribus!</p>

    </div>

</div>

<?php include('partials/site-footer.php'); ?>
<?php include('partials/wvu-footer.php'); ?>
<?php include('partials/footer.php'); ?>